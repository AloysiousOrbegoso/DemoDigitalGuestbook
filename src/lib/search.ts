import type { Block, GuideContent, Page } from "../types/guide";
...
function blockStrings(block: Block): string[] {
  switch (block.type) {
    case "text": return [block.heading ?? "", block.body];
    case "steps": return [block.heading ?? "", ...block.steps];
    case "image": return [block.alt, block.caption ?? ""];
    case "video": return [block.title];
    case "link": return [block.label, block.description ?? ""];
    case "wifi": return ["wifi wi-fi internet password network", block.network, block.note ?? ""];
    case "contact": return [block.heading ?? "", ...block.methods.flatMap((m) => [m.label, m.detail ?? "", m.value])];
    case "map-link": return [block.label, block.note ?? ""];
    case "list": return [block.heading ?? "", ...block.items.flatMap((i) => [i.title, i.detail ?? ""])];
  }
}

function pageStrings(page: Page, content: GuideContent): string[] {
  const extra: string[] = [];
...
export function searchGuide(content: GuideContent, query: string): SearchResult[] {
  ...
  // Title matches first.
  return results.sort(
    (a, b) => Number(b.pageTitle.toLowerCase().includes(terms[0])) - Number(a.pageTitle.toLowerCase().includes(terms[0])),
  );
}
