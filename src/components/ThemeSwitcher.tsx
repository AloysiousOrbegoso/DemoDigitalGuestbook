...
const swatchBg = (t: ThemeDef) => `linear-gradient(135deg, ${t.swatch[0]} 50%, ${t.swatch[1]} 50%)`;
 ...
 */
export function ThemeSwitcher({ placement = "desktop" }: { placement?: "desktop" | "mobile" }) {
  ...
  return (
    <div className={`theme-switcher theme-switcher--${placement}`} role="radiogroup" aria-label="Colour theme">
      <span className="theme-switcher__label" aria-hidden="true">{current.label}</span>
      {themes.map((t) => (
        <button
          key={t.id}
          type="button"
